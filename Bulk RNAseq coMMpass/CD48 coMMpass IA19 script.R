...
temp_bar <- data.frame(y = as.numeric(temp[temp$GENE == "CD48", 1:203]), 
                       x = x,                
                       stringsAsFactors = F)

summary(temp_bar%>%filter(x==2))
...
# PLOT variance of CD48 expression across every sample
temp_bar%>%ggplot(aes(x = x, y = y)) + 
  ...
  ggtitle(label = "CD48", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD48"], 3)))+
...
temp%>%filter(GENE == "CD48")
names(temp)
temp[temp$GENE == "CD48", 1:162]
x<-str_extract(names(temp), "_[0-9]_")[1:162] 
x<-str_extract(x, "[0-9]")[1:162]
x

temp_bar <- data.frame(y = as.numeric(temp[temp$GENE == "CD48", 1:162]), 
                       x = x,                
                       stringsAsFactors = F)
temp_bar
...
temp_bar1%>%ggplot(aes(x = x, y = y)) + 
  ...
  ggtitle(label = "CD48", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD48"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD48"], 3))) +
  theme_minimal()
...
temp_flip<-temp%>%filter(GENE == "CD48")
head(temp_flip)
...
temp_bar2%>%ggplot(aes(ISS, temp_flip)) + 
  ...
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on ISS stage")+
  #ggpubr::stat_pvalue_manual(stat_pvalue, label = "p.signif") +
  theme_minimal()
...
translocations_df<-data.frame(`SeqFISH.Files_MMRF_CoMMpass_IA19_genome_tumor_only_mm_igtx_pairoscope`)
...
seqFISH<-data.frame(`SeqFISH.Files_MMRF_CoMMpass_IA19_genome_gatk_cna_seqFISH`)
...
# Determing significant p-values for showing it up in the plot only the significant ones
stat_pvalue <- temp_bar2 %>% 
  rstatix::wilcox_test(temp_flip ~ NDS2) %>%
  filter(p < 0.05) %>% 
  rstatix::add_significance("p") %>% 
  rstatix::add_y_position() %>% 
  mutate(y.position = seq(min(y.position), max(y.position),length.out = n()))
...
# Assuming res is a data frame
p_value <- res$pvalue[res$GENE %in% "CD48"]
...
temp_bar2%>%ggplot(aes( x = NDS2, y = temp_flip)) + 
  ...
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on t(4:14) [NDS2] status", 
          subtitle = sprintf("log2FC = %s   P-value = %s",
                             round(res$log2FoldChange[res$GENE %in% "CD48"], 3),
                             ifelse(p_value < 0.0001, "<0.0001", formatC(p_value, format = "e", digits = 3)))) +
  theme_minimal()
...
# Determing significant p-values for showing it up in the plot only the significant ones
stat_pvalue <- temp_bar2 %>% 
  rstatix::wilcox_test(temp_flip ~ CCND1) %>%
  filter(p < 0.05) %>% 
  rstatix::add_significance("p") %>% 
  rstatix::add_y_position() %>% 
  mutate(y.position = seq(min(y.position), max(y.position),length.out = n()))
...
temp_bar2%>%ggplot(aes( x = CCND1, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(11:14) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on t(11:14) [CCND1] status", 
          subtitle = sprintf("log2FC = %s   P-value = %s",
                             round(res$log2FoldChange[res$GENE %in% "CD48"], 3),
                             ifelse(p_value < 0.0001, "<0.0001", formatC(p_value, format = "e", digits = 3)))) +
  theme_minimal()
...
# Determing significant p-values for showing it up in the plot only the significant ones
stat_pvalue <- temp_bar2 %>% 
  rstatix::wilcox_test(temp_flip ~ CCND3) %>%
  filter(p < 0.05) %>% 
  rstatix::add_significance("p") %>% 
  rstatix::add_y_position() %>% 
  mutate(y.position = seq(min(y.position), max(y.position),length.out = n()))
...
temp_bar2%>%ggplot(aes( x = CCND3, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(6:14) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on t(6:14) [CCND3] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD48"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD48"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = MYC, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "MYC mutation",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on MYC mutation", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE %in% "CD48"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE %in% "CD48"], 3))) +
  theme_minimal()
...
# Determing significant p-values for showing it up in the plot only the significant ones
stat_pvalue <- temp_bar2 %>% 
  rstatix::wilcox_test(temp_flip ~ MAF) %>%
  filter(p < 0.05) %>% 
  rstatix::add_significance("p") %>% 
  rstatix::add_y_position() %>% 
  mutate(y.position = seq(min(y.position), max(y.position),length.out = n()))
...
temp_bar2%>%ggplot(aes( x = MAF, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(14;16) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on t(14;16) [c-MAF] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD48"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD48"], 3))) +
  theme_minimal()
...
# Determing significant p-values for showing it up in the plot only the significant ones
stat_pvalue <- temp_bar2 %>% 
  rstatix::wilcox_test(temp_flip ~ MAFB) %>%
  filter(p < 0.05) %>% 
  rstatix::add_significance("p") %>% 
  rstatix::add_y_position() %>% 
  mutate(y.position = seq(min(y.position), max(y.position),length.out = n()))
...
temp_bar2%>%ggplot(aes( x = MAFB, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(14;20) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on t(14;20) [MAFB] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD48"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD48"], 3))) +
  theme_minimal()
...
# Determing significant p-values for showing it up in the plot only the significant ones
stat_pvalue <- temp_bar2 %>% 
  rstatix::wilcox_test(temp_flip ~ del17p13) %>%
  filter(p < 0.05) %>% 
  rstatix::add_significance("p") %>% 
  rstatix::add_y_position() %>% 
  mutate(y.position = seq(min(y.position), max(y.position),length.out = n()))
...
temp_bar2%>%ggplot(aes( x = del17p13, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "del17p13 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on del17p13 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE %in% "CD48"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE %in% "CD48"], 3))) +
  theme_minimal()
...
# Determing significant p-values for showing it up in the plot only the significant ones
stat_pvalue <- temp_bar2 %>% 
  rstatix::wilcox_test(temp_flip ~ TP53) %>%
  filter(p < 0.05) %>% 
  rstatix::add_significance("p") %>% 
  rstatix::add_y_position() %>% 
  mutate(y.position = seq(min(y.position), max(y.position),length.out = n()))
...
temp_bar2%>%ggplot(aes( x = TP53, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "TP53 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on TP53 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE %in% "CD48"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE %in% "CD48"], 3))) +
  theme_minimal()
...
# Determing significant p-values for showing it up in the plot only the significant ones
stat_pvalue <- temp_bar2 %>% 
  rstatix::wilcox_test(temp_flip ~ Gain1q21) %>%
  filter(p < 0.05) %>% 
  rstatix::add_significance("p") %>% 
  rstatix::add_y_position() %>% 
  mutate(y.position = seq(min(y.position), max(y.position),length.out = n()))
...
temp_bar2%>%ggplot(aes( x = Gain1q21, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "Gain1q21 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on Gain1q21 status", 
                  subtitle = sprintf("log2FC = %s   P-value = %s",
                                     round(res$log2FoldChange[res$GENE %in% "CD48"], 3),
                                     ifelse(p_value < 0.0001, "<0.0001", formatC(p_value, format = "e", digits = 3)))) +
  theme_minimal()
...
# Determing significant p-values for showing it up in the plot only the significant ones
stat_pvalue <- temp_bar2 %>% 
  rstatix::wilcox_test(temp_flip ~ del1p22) %>%
  filter(p < 0.05) %>% 
  rstatix::add_significance("p") %>% 
  rstatix::add_y_position() %>% 
  mutate(y.position = seq(min(y.position), max(y.position),length.out = n()))
...
temp_bar2%>%ggplot(aes( x = del1p22, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "del1p22 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on del1p22 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE %in% "CD48"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE %in% "CD48"], 3))) +
  theme_minimal()
...
# Determing significant p-values for showing it up in the plot only the significant ones
stat_pvalue <- temp_bar2 %>% 
  rstatix::wilcox_test(temp_flip ~ MYC) %>%
  filter(p < 0.05) %>% 
  rstatix::add_significance("p") %>% 
  rstatix::add_y_position() %>% 
  mutate(y.position = seq(min(y.position), max(y.position),length.out = n()))
...
temp_bar2%>%ggplot(aes( x = MYC, y = temp_flip)) + 
  ...
  geom_jitter(shape = 16, 
              ...
              alpha = 0.4,
              color = "red") +
  scale_x_discrete(name = "MYC Status by seqFISH",
                   breaks = 0:1,
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD48 expression based on MYC status (SeqFISH)", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD48"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD48"], 3))) +
  theme_minimal()
...
raw <- data.frame(Expression.Estimates...Gene.Based_MMRF_CoMMpass_IA19_star_geneUnstranded_counts)
ncol(raw)
raw<-raw%>%filter(Gene == "ENSG00000117091")
raw
counts <- as.matrix(raw[-1])
...
counts2
sort(counts2$Count, decreasing = FALSE)  # min 54 and max 34197 (20% max = 27357  // 20% min <6839)
...
topbottomsurv<-topbottomsurv%>%mutate("Percentage" = case_when(Count<2844 ~ "Bottom", 
                                                Count>10028 ~ "Top"))
...
topbottomsurv<-topbottomsurv%>%mutate("Percentage" = case_when(Count<5747 ~ "Bottom", 
                                                               Count>5769 ~ "Top"))
