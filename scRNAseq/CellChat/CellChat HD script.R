...
# ========================================
# 1. Load and Subset Seurat Object (HD only)
# ========================================
...
# Subset MM cells
hd.cells <- rownames(seurat.obj@meta.data[grepl("HD", seurat.obj@meta.data$Patient), ])
seurat.hd <- subset(seurat.obj, cells = hd.cells)

# ========================================
# 2. Ensure proper cell type annotation
# ========================================
# Only run this if your cell types were renamed manually in a previous step
if (!"celltype" %in% colnames(seurat.hd@meta.data)) {
  seurat.hd$celltype <- Idents(seurat.hd)
}

# Set identity to cell type
Idents(seurat.hd) <- seurat.hd$celltype

# ========================================
# 3. Build CellChat Object
# ========================================
data.input <- GetAssayData(seurat.hd[["RNA"]], layer = "data")
meta <- seurat.hd@meta.data
meta$cell_type <- as.character(Idents(seurat.hd))  # Ensure it's character
...
# Seurat violin plot
VlnPlot(seurat.hd, features = c("CD48", "CD244"), group.by = "celltype")
